...
(function(App) {
  ...
  function renderSessionCard(session) {
    ...
    const subjectDisplay = subjects.length > 0 
      ? subjects.slice(0, 3).join(', ') + (subjects.length > 3 ? ` +${subjects.length - 3} more` : '')
      : '';
    
    // Get additional stats
    const duration = session.config?.duration || 90;
    const totalQs = stats.total || session.config?.totalQuestions || 0;
    const accuracy = stats.accuracy || (stats.total - stats.unanswered > 0 ? ((stats.correct / (stats.total - stats.unanswered)) * 100).toFixed(1) : 0);
    ...
    return `
      <div class="session-card">
        ...
        ${subjectDisplay ? `<div class="session-subjects" style="font-size: 12px; color: var(--color-text-secondary); margin: 8px 0; padding: 6px 10px; background: var(--color-secondary); border-radius: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${subjectDisplay}</div>` : ''}
        <div class="session-stats-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin: 10px 0; font-size: 11px; text-align: center;">
          <div style="padding: 6px; background: rgba(6, 214, 160, 0.1); border-radius: 6px;"><span style="color: var(--color-success); font-weight: 600;">${stats.correct}</span><br><span style="color: var(--color-text-secondary);">Correct</span></div>
          <div style="padding: 6px; background: rgba(255, 84, 89, 0.1); border-radius: 6px;"><span style="color: var(--color-error); font-weight: 600;">${stats.incorrect}</span><br><span style="color: var(--color-text-secondary);">Wrong</span></div>
          <div style="padding: 6px; background: rgba(244, 162, 97, 0.1); border-radius: 6px;"><span style="color: var(--color-warning); font-weight: 600;">${stats.unanswered}</span><br><span style="color: var(--color-text-secondary);">Skipped</span></div>
          <div style="padding: 6px; background: rgba(185, 28, 28, 0.1); border-radius: 6px;"><span style="color: var(--color-primary); font-weight: 600;">${accuracy}%</span><br><span style="color: var(--color-text-secondary);">Accuracy</span></div>
        </div>
    ...
    sessions.forEach(session => {
      session.questions?.forEach(q => {
        ...
        if (difficultyData[diff]) {
          ...
          // FIX: Use stored isCorrect value instead of index comparison
          if (q.isCorrect === true) {
  ...
  function viewSessionResults(sessionId) {
    ...
    // FIX: Preserve session's stored question data (includes shuffled options and correct indices)
    // Only supplement with explanation from fullQ if missing
    state.examQuestions = session.questions.map(q => {
      const fullQ = state.allQuestions.find(fq => fq.id === q.id);
      return {
        ...q,
        explanation: q.explanation || (fullQ?.explanation || '')
      };
    });
    ...
    // FIX: Use stored evaluatedAnswers from the session's questions
    state.evaluatedAnswers = session.questions.map(q => ({
      questionId: q.id,
      selectedOption: q.userAnswer,

...
(function(App) {
  ...
  function viewResultsInExamTab(sessionId) {
    ...
    // CLEAR INCOMPATIBLE STATE FIRST - ensure clean render
    state.currentScreen = 'results';
    state.appMode = App.APP_MODES?.EXAM_RESULTS || 'results';
    state.isExamModeActive = false;
    state.examQuestions = [];
    state.userAnswers = [];
    state.evaluatedAnswers = [];
    state.results = {};
    
    // Exit any active exam mode
    if (typeof exitFullExamMode === 'function') {
      exitFullExamMode();
    }
    ...
    // Wait for DOM to settle then load session
    setTimeout(() => {
      ...
      // Calculate stats from STORED evaluation data (read-only)
      state.results = App.Storage.calculateSessionStats(session);
      state.results.submitted = true;
      
      // LOAD QUESTIONS VERBATIM from session - stored snapshot is authoritative
      // Only fallback to fullQ for display-only fields (explanation) if missing
      ...
      // Clear evaluated answers since we're using stored data
  ...
  function performLandingNavigation() {
    ...
    // FULL FLOW RESET - reset navigation flow, NOT data
    // Preserve: performance history, attempt data, analytics (in localStorage)
    // Reset: UI state, exam state, mode
    state.currentScreen = 'landing';
    state.appMode = App.APP_MODES?.LANDING || 'landing';
    state.isExamModeActive = false;
    state.examQuestions = [];
    state.userAnswers = [];
    state.evaluatedAnswers = [];
    ...
    // RESET FLOW STATE - configuration must start clean
    ...
    // Reset branch/exam selection UI

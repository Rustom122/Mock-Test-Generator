...
(function(App) {
  ...
  // MODE AUTHORITY: Single source of truth for app state
  const APP_MODES = {
    LANDING: 'landing',
    CONFIG: 'config', 
    EXAM_CONDUCTION: 'exam',
    EXAM_RESULTS: 'results',
    PERFORMANCE_VIEW: 'performance'
  };

  App.APP_MODES = APP_MODES;

  App.appState = {
    currentScreen: 'config',
    appMode: APP_MODES.CONFIG,
  ...
  async function fetchQuestions() {
    ...
    return parsed.data.map((row, idx) => {
      ...
      if (correctStr) {
        ...
        if (cleaned === 'A' || cleaned === '0') correctIndex = 0;
        else if (cleaned === 'B' || cleaned === '1') correctIndex = 1;
        else if (cleaned === 'C' || cleaned === '2') correctIndex = 2;
        else if (cleaned === 'D' || cleaned === '3') correctIndex = 3;
      }
      ...
      const status = findColumn(row, ['Status', 'status', 'Active', 'active', 'Enabled', 'enabled']);
      const isAvailable = !status || status.toString().trim().toLowerCase() === 'yes' || status.toString().trim().toLowerCase() === 'true' || status.toString().trim() === '1';
    ...
    }).filter(q => q.question && q.question.trim() !== '' && q.status === true);
  }
  ...
  function prepareExamQuestions(questions) {
    return questions.map(q => {
      // Create stable option identities with unique IDs
      ...
      const correctOptionId = shuffledOptions[newCorrectIndex]?.id || null;
      
      return {
        ...q,
        options: shuffledOptions.map(opt => opt.text),
        optionIds: shuffledOptions.map(opt => opt.id),

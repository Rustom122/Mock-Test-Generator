...
(function(App) {
  ...
  function createSessionObject(config) {
    ...
    // LOCK EVALUATION AT SUBMISSION TIME - evaluate once, store forever
    const evaluatedQuestions = state.examQuestions.map((q, i) => {
      const userAnswer = state.userAnswers[i]?.selectedOption;
      const correctAnswer = q.correctAnswer;
      const selectedOptionId = (userAnswer !== null && q.optionIds) ? q.optionIds[userAnswer] : null;
      const correctOptionId = q.correctOptionId || null;
      
      // ID-based correctness evaluation (if IDs available) or index-based fallback
      let isCorrect = false;
      if (userAnswer !== null) {
  ...
  function calculateSessionStats(session) {
    ...
    session.questions.forEach(q => {
      ...
      if (q.userAnswer === null || q.userAnswer === undefined) {
        ...
        // USE STORED EVALUATION - never recompute correctness
        const wasCorrect = q.isCorrect !== undefined ? q.isCorrect : (q.userAnswer === q.correctAnswer);
        
        // SELF-VALIDATION CHECK
        if (q.selectedOptionId && q.correctOptionId) {
          const shouldBeCorrect = q.selectedOptionId === q.correctOptionId;
          if (wasCorrect !== shouldBeCorrect) {
            console.error('[CORRECTNESS MISMATCH] Question:', q.id, 
              'Stored:', wasCorrect, 'Computed:', shouldBeCorrect,
              'SelectedID:', q.selectedOptionId, 'CorrectID:', q.correctOptionId);
          }
        }
  ...
  function getIncorrectQuestionIds(sessionId) {
    ...
    return session.questions
      .filter(q => q.userAnswer !== null && q.userAnswer !== undefined && q.isCorrect === false)
      .map(q => q.id);
  }

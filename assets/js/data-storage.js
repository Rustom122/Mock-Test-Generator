...
(function(App) {
  ...
  function getAttemptNumber(session) {
    const sessions = getSessions();
    const configKey = `${session.mode}_${session.branch}_${session.examType}_${JSON.stringify(session.config.subjects || [])}`;
    let count = 0;
    
    for (const s of sessions) {
      const sKey = `${s.mode}_${s.branch}_${s.examType}_${JSON.stringify(s.config.subjects || [])}`;
      if (sKey === configKey) count++;
